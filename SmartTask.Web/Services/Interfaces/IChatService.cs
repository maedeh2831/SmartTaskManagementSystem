...
public interface IChatService
{
    ...
    Task<List<ChatMessageViewModel>> SearchAsync(int projectId, string term, int take = 50, int skip = 0);
    ...
    Task<int?> DeleteMessageAsync(int messageId, int userId, IFileUploadService? fileUploadService = null);
    ...
    Task<ChatMessageViewModel?> ToggleReactionAsync(int messageId, int userId, string emoji);

    // ===== Pin =====

    Task<ChatMessageViewModel?> TogglePinAsync(int messageId, int userId);

    Task<List<ChatMessageViewModel>> GetPinnedMessagesAsync(int projectId);

    // ===== Rate Limiting =====

    Task<bool> CanSendMessageAsync(int projectId, int userId);
}
