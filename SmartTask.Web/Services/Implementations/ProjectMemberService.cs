...
public class ProjectMemberService : IProjectMemberService
{
    ...
    public ProjectMemberService(ApplicationDbContext context, INotificationService notificationService)
    {
        _context = context;
    ...
    public async Task AddMemberAsync(int projectId, int userId, ProjectRoleType role)
    {
        ...
        if (existing != null)
        {
            ...
            await NotifyMemberAddedAsync(projectId, userId, role);
            return;
        }
    ...
    private async Task NotifyMemberAddedAsync(int projectId, int userId, ProjectRoleType role)
    {
        ...
        await _notificationService.CreateAsync(
            userId,
            "افزودن به پروژه",
            $"شما با نقش «{role}» به پروژه «{project.Name}» اضافه شدید.",
