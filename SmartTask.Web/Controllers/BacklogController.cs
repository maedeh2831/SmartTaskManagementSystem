...
public class BacklogController : BaseController
{
    ...
    public async Task<IActionResult> Index(int projectId)
    {
        ...
        // Load task assignments to get assignee names
        ...
        var vm = new BacklogIndexViewModel
        {
            ...
            Sprints = sprints.Select(sprint => new SprintGroupViewModel
            {
                ...
                Stories = sprint.UserStories.Select(x => new UserStoryListItemViewModel
                {
                    ...
                    Tasks = x.Tasks.Select(t => new BacklogTaskItemViewModel
                    {
                        Id = t.Id,
                        Title = t.Title,
                        Status = t.Status,
                        ...
                        Priority = t.Priority,
                        Estimate = t.Estimate,
                        AssigneeName = taskAssignments.FirstOrDefault(a => a.TaskItemId == t.Id)?.ApplicationUser?.FullName
                    }).ToList()
                }).ToList()
            }).ToList(),
