...
- `Configurations/` — 34 فایل پیکربندی EF Core برای هر موجودیت
...
**پیاده‌سازی فنی (موجود در سرویس):**

| جزئیات | مقدار |
|--------|-------|
| **متد** | `GetDependencyGraphAsync()` در `TaskDependencyService` |
...
1. `TaskBreakdownService` — تجزیه تسک
2. `DelayRiskService` — تحلیل متنی ریسک
3. `SprintReportAiService` — گزارش اسپرینت
...
│   └── Configurations/               # 34 فایل Fluent API Configuration
│
├── Hubs/
...
│
├── Models/
│   ├── Entities/                      # 34 موجودیت داده‌ای
│   ├── Enums/                         # 23 شمارشگر
│   ├── ViewModels/                    # 30+ پوشه ViewModel
│   ├── DTOs/                          # BurndownPointDto, VelocityPointDto
│   └── Navigation/                    # مدل‌های ناوبری
│
├── Services/
│   ├── AI/
│   │   ├── IAiClientService.cs        # رابط AI
│   │   ├── AiClientService.cs         # پیاده‌سازی AI (HTTP به LM Studio)
...
| Task Trading | ✅ تکمیل |
| Dependency Graph (بصری) | 🔄 در حال پیاده‌سازی |
